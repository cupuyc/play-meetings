  ...
  function connect () {
    try {
      ...
      socket.onopen = function(evt) {
        ...
        connected = true;
        send({ messageType: 'change', size: size, color: color, name: pname });
      };
...
(function(){
  ...
  onSocketMessage = function (e) {
    ...
    if (m.messageType=="youAre") pid = m.pid;
    
    if (m.messageType != "disconnect") {
      if (m.messageType == "trace") {
...
(function(){
  ...
  function sendPoints () {
    lastSent = +new Date();
    send({ messageType: "trace", points: points, num: numTrace });
    points = [];
  }
  function sendMove (x, y) {
    lastSent = +new Date();
    send({ messageType: "move", x: x, y: y });
  }
